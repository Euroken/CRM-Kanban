...
// Menu Popover Component (No changes needed here)
...
// Contact Hover Card Component (No changes needed here, but it won't be triggered from the deal card anymore)
...
export const SortableDealCard = ({ deal, currentUser, month }) => {
  ...
  // Removed state and handlers for contact card as it's no longer triggered from here
  ...
  // Removed contact related handlers
  ...
  return (
    <>
      <div
        ref={setNodeRef}
        style={style}
        className={`bg-white rounded-lg shadow-sm border p-3 mb-2 hover:shadow-md transition-all relative ${
      ...
      >
        ...
        <div
          {...attributes}
          {...listeners}
          className="absolute left-1 top-1 p-1 cursor-grab active:cursor-grabbing text-gray-400 hover:text-gray-600"
        >
        ...
        <div className="ml-5">
          ...
          <div className="mt-2 flex justify-end items-center"> {/* Adjusted for no contact button */}
            <div className="text-xs text-gray-500">
              Stage: <span className="font-medium">{deal.stage || deal.probability}</span>
            </div>
            {/* Removed contact button entirely */}
            {/*
            <button
              onClick={handleContactClick}
              onMouseEnter={handleContactMouseEnter}
              onMouseLeave={handleContactMouseLeave}
              className="w-7 h-7 bg-gray-200 rounded-full flex items-center justify-center text-xs font-medium hover:bg-blue-100 hover:text-blue-600 transition-colors"
            >
      ...
      {/* The Contact Hover Card component is still defined, but it won't be shown
          unless another part of your application triggers it. */}
      {/*
      <ContactHoverCard
